...
def get_confusion_matrix_exclude(tau_arr, g_perprotein, pred, toi_perprotein, n_gt, ic_arr=None, B_ind = None):
    ...
    for i, tau in enumerate(tau_arr):
        ...
        # Terms subsets
        TP = [np.logical_and(p_i, g_i) for p_i, g_i in zip(p_perprotein, g_perprotein)]  # TP
        FP = [np.logical_and(p_i, np.logical_not(g_i)) for p_i, g_i in zip(p_perprotein, g_perprotein)]  # FP, predicted but not in the ground truth
        FN = [np.logical_and(np.logical_not(p_i), g_i) for p_i, g_i in zip(p_perprotein, g_perprotein)]  # FN, not predicted but in the ground truth

        # Weighted evaluation
        if ic_arr is not None:
            p_perprotein = [p_i * ic_arr[tois] for p_i, tois in zip(p_perprotein, toi_perprotein)]
            TP = [inter * ic_arr[tois] for inter, tois in zip(TP, toi_perprotein)]  # TP
            FP = [misinf * ic_arr[tois] for misinf, tois in zip(FP, toi_perprotein)]  # FP, predicted but not in the ground truth
            FN = [rem * ic_arr[tois] for rem, tois in zip(FN, toi_perprotein)]  # FN, not predicted but in the ground truth

        CM['p'].append(p_perprotein)
        CM['TP'].append(TP)
        CM['FP'].append(FP)
        CM['FN'].append(FN)
...
def compute_metrics(pred, gt, tau_arr, toi, gt_exclude=None, ic_arr=None, n_cpu=0, B = 0, B_pct = 0):
    """
    ...
    if gt_exclude is None:
        ...
        with mp.Pool(processes=n_cpu) as pool:
            results = pool.starmap(get_confusion_matrix_exclude, arg_lists)
...
def evaluate_prediction(prediction, gt, ontologies, tau_arr, gt_exclude=None, normalization='cafa', n_cpu=0, B = 0, B_pct = 0):
    ...
    for ns in prediction:
        if gt_exclude is None:
            ...
            toi_perprotein = [np.setdiff1d(ontologies[ns].toi, gt_exclude[ns].matrix[p, :].nonzero()[0], assume_unique=True) for p in
                range(num_proteins)]
...
def cafa_eval(obo_file, pred_dir, gt_file, ia=None, no_orphans=False, norm='cafa', prop='max',
    ...
    for file_name in pred_files:
        ...
        if not prediction:
            ...
            df_pred, metrics_B = evaluate_prediction(prediction, gt, ontologies, tau_arr, gt_exclude,
...
def write_results(df, dfs_best,  metrics_B_df, out_dir='results', th_step=0.01):
    ...
    if isinstance(metrics_B_df, pd.DataFrame):
        metrics_B_df.to_csv('{}/bootstrap.tsv'.format(out_folder), float_format="%.{}f".format(decimals),
                                     sep="\t")
